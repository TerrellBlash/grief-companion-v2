...
export default function CandleRitual() {
  ...
  return (
    <div className="h-[100dvh] bg-[#1F1D24] relative overflow-hidden flex flex-col items-center justify-between py-6">
      <div className="absolute inset-0 noise-texture opacity-10" />

      {/* Pulsating Background Halo */}
      <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] rounded-full bg-[var(--color-amber)]/10 blur-[80px] transition-all duration-[4000ms] ease-in-out ${ritualStarted && breathIn ? 'scale-150 opacity-20' : 'scale-100 opacity-10'}`} />
      ...
      <div className="flex-1 flex flex-col items-center justify-center w-full px-6 overflow-y-auto no-scrollbar relative z-10">
        {!ritualStarted ? (
          ...
          <div className="text-center mb-10 animate-enter absolute top-16 left-0 right-0">
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--color-clay)] mb-2">In Loving Memory</p>
            <h2 className="font-serif text-3xl text-[#EBE8E1] tracking-wide">For {dedication}</h2>
          </div>
        )}

        {/* Name Input */}
        <div className={`w-full max-w-[280px] mb-8 transition-all duration-700 absolute top-[220px] z-20 ${!ritualStarted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10 pointer-events-none'}`}>
        ...
        {/* Candle */}
        <div className={`relative mb-6 shrink-0 transition-transform duration-1000 flex flex-col items-center ${ritualStarted ? 'scale-110 mt-12' : 'scale-90 mt-[180px]'}`}>

          {/* Pulsating Ring */}
          <div className={`absolute top-[40%] left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full border border-[var(--color-amber)]/20 transition-all duration-[4000ms] ease-in-out ${ritualStarted && breathIn ? 'w-80 h-80 opacity-60 border-[var(--color-amber)]/40' : 'w-40 h-40 opacity-0'}`} />
          ...
          <div className="w-16 h-40 bg-gradient-to-t from-[#2A2730] to-[#45414D] rounded-t-xl rounded-b-lg relative mx-auto shadow-2xl z-20">
            ...
            {/* Flame */}
            <div className={`absolute -top-16 left-1/2 -translate-x-1/2 w-8 h-16 bg-gradient-to-t from-[var(--color-amber)] via-[#E8B991] to-white rounded-[50%_50%_50%_50%_/_60%_60%_40%_40%] mix-blend-screen filter blur-[1px] transition-all duration-700 ${ritualStarted ? 'animate-flame opacity-90 scale-100' : 'opacity-0 scale-50'}`} />

            {/* Glow */}
            <div className={`absolute -top-20 left-1/2 -translate-x-1/2 w-32 h-32 bg-[var(--color-amber)] rounded-full filter blur-[40px] transition-all duration-1000 ${ritualStarted ? 'opacity-30' : 'opacity-0'}`} />
          </div>

          {/* Breathing Text */}
          <div className={`mt-8 text-center transition-opacity duration-1000 h-6 ${ritualStarted ? 'opacity-100' : 'opacity-0'}`}>
