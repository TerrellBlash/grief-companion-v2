...
import { Settings, Flame, Users, ArrowRight, X, HeartHandshake, BookHeart, Sparkles, Mail } from 'lucide-react';

// MenuCard Component - Exact Aura.build style
const MenuCard = ({
  title,
  subtitle,
  icon: Icon,
  ...
  >
    ...
    <div className="w-11 h-11 rounded-full bg-white flex items-center justify-center text-[var(--text-main)] shadow-sm shrink-0 border border-[var(--color-sand)]/20">
      ...
      <header className="pt-14 px-8 mb-6 relative z-10 animate-enter flex justify-between items-start">
        <div className="flex-1">
          ...
          <h1 className="font-serif text-[32px] leading-[1.1] text-[var(--text-main)] tracking-tight">
            <span className="opacity-70 font-normal">Welcome home,</span><br/>
      ...
      <div className="px-5 flex flex-col gap-4 relative z-10">
        ...
        {showNudge && (
          <div className="bg-[var(--bg-card)] border border-[var(--color-sand)]/40 rounded-[24px] p-4 flex items-center justify-between shadow-[0_4px_20px_-4px_rgba(158,88,77,0.1)] relative overflow-hidden group animate-enter">
            ...
            <button onClick={() => setShowNudge(false)} className="w-8 h-8 flex items-center justify-center text-[var(--text-muted)] hover:text-[var(--color-clay)] transition-colors relative z-10">
              <X size={16} />
            </button>
          </div>
        )}

        {/* Daily Ritual Card - Gradient Background */}
        <Link
          href="/rituals/candle"
        ...
        >
          ...
          <div className="relative z-10 flex justify-between items-end mt-4">
            <div>
              <h2 className="font-serif text-[36px] leading-[1.1] text-white mb-1.5 tracking-tight">Light a<br/>Candle</h2>
          ...
          <MenuCard
            ...
            customIcon={
              <div className="flex items-center justify-center -space-x-2">
                <div className="w-7 h-7 rounded-full bg-[var(--color-sand)] border-2 border-white flex items-center justify-center text-[8px] font-bold text-[var(--text-main)]">S</div>
                <div className="w-7 h-7 rounded-full bg-[var(--color-clay)] border-2 border-white flex items-center justify-center text-[8px] font-bold text-white">M</div>
                <div className="w-7 h-7 rounded-full bg-[var(--color-amber)] border-2 border-white flex items-center justify-center text-[8px] font-bold text-white">K</div>
              </div>
            }
          />
        </div>
